...
`syntenet` is a package that can be used to infer synteny networks from
...
written in C++, using the Rcpp [@eddelbuettel2011rcpp] framework 
for R and C++ integration. This means that `syntenet` comes with a native 
...
In the edge list above, for example, the genes `net[1,1]` and `net[1,2]` are
...
[^2]: **Friendly tip:** `syntenet` uses the *Infomap* algorithm
...
CliquePercolation [@lange2021cliquepercolation].
